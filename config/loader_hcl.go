...
func parseDevices(result *Config, list *ast.ObjectList) error {
	...
	collection := make([]*DeviceInfo, 0, len(list.Items))
	...
	for _, item := range list.Items {
		...
		if _, ok := seen[n]; ok {
			return fmt.Errorf("class '%s' defined more than once", n)
		}
		seen[n] = struct{}{}
		...
		if ot, ok := item.Val.(*ast.ObjectType); ok {
			...
			return fmt.Errorf("class '%s': should be an object", n)
		}
		* /
		...
		mapstructure.WeakDecode(m, &obj)

		collection = append(collection, &obj)
	}

	// Set the results
	result.Devices = collection
	return nil
}


// checkHCLKeys checks for invalid keys
// Taken from https://github.com/hashicorp/hcl/issues/109
